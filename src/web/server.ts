...
💰 <span class="m">@degen</span> earns ${pct(fee.share.deployer)} of every trading fee</div>
      ...
      <section class="section faq" id="faq">
        ...
        <details><summary>Why did the bot say my ticker is reserved?</summary><p>That ticker was launched on Long.xyz (or via LONGSHOT) in the last ${cfg.rules.tickerCooldownHours > 0 ? `${Math.round(cfg.rules.tickerCooldownHours / 24)} days` : "past"}, or it's a real Robinhood stock symbol. Pick another ticker, or try again once the ticker is free.</p></details>
